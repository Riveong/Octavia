<script lang="ts">
  ...
  async function eksporJson() {
    ...
    try {
      ...
      const path = await Neutralino.os.showSaveDialog("Ekspor data produk", {
        defaultPath: `ocashy-produk-${new Date().toISOString().slice(0, 10)}.json`,
        filters: [{ name: "JSON", extensions: ["json"] }],
      });
      if (!path) return;
      await Neutralino.filesystem.writeTextFile(path, JSON.stringify(rows, null, 2));
  ...
  async function imporJson() {
    busy = true;
    try {
      const pathEntry = await Neutralino.os.showOpenDialog("Impor data produk (JSON)", {
        filters: [{ name: "JSON", extensions: ["json"] }],
        multiSelections: false
      });
      const path = Array.isArray(pathEntry) ? pathEntry[0] : pathEntry;
      if (!path) return;
      const text = await Neutralino.filesystem.readTextFile(path);
