 ...
 * %APPDATA%/ocashy/receipts/, lalu (opsional) buka dengan viewer default
...
export async function cetakNota(data: NotaLengkap, s: PrintSettings): Promise<string> {
  ...
  const receiptsFolder = `${appFolder}/receipts`;

  // Pastikan folder exist
  try {
    await Neutralino.filesystem.createDirectory(receiptsFolder);
  } catch (err: any) {
  ...
  const fullPath = `${receiptsFolder}/${fileName}`;

  await Neutralino.filesystem.writeBinaryFile(fullPath, bytes.buffer);

  if (s.silentPrint) {
    try {
      const executable = s.sumatraPath && s.sumatraPath.trim() !== ""
        ? s.sumatraPath.trim()
        : "SumatraPDF.exe";

      let cmd = `"${executable}"`;
      if (s.printerName && s.printerName.trim() !== "") {
        cmd += ` -print-to "${s.printerName.trim()}"`;
      } else {
        cmd += " -print-to-default";
      }
      cmd += ` -print-settings "noscale" "${fullPath}"`;
  ...
  if (s.autoOpen) {
    await Neutralino.os.open(fullPath);
  }
  return fullPath;
}
